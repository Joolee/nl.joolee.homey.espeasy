...
module.exports = class P1 extends GeneralDevice {
	...
	connect() {
		if (!this.p1.connecting) {
			...
			this.setUnavailable(this.errorMsg ? this.errorMsg : Homey.__("p1.connecting", { "port": this.port }));
			this.p1.connect({
				"host": this.unit.ip,
				"port": this.port
			});
	...
	onData(data) {
		if (data.toString().substr(0, 4) == "HTTP") {
			...
			this.errorMsg = Homey.__("p1.connected_to_http_server", { "port": this.port });
		...
		// The ESP should already have checked this already anyway
		...
		if (!dg.electricity.received.actual.reading) {
			...
			this.setUnavailable(Homey.__("p1.invalid_datagram", { "port": this.port }));
			return;
		}
