...
module.exports = class SensorDevice extends GeneralDevice {
	...
	updateTelemetry(reason, recurse) {
		try {
			...
			let metrics = {
				"Task plugin": `${this.task.TaskDeviceNumber} - ${this.task.Type}`,
			...
			Homey.app.sendTelemetry('Sensor', reason, `/device/sensor/${metrics["Task driver"]}/${metrics["Task plugin"]}`, metrics);
		} catch (error) {
	...
	setValue(key, value, source = 'unknown') {
		...
		const oldValue = this.getCapabilityValue();
