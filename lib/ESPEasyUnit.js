...
module.exports = class ESPEasyUnit extends Homey.SimpleClass {
	...
	sendCommand(commandList, callback = () => { }, silent = false) {
		...
		http.get({
			...
			.then((response) => {
				...
				} else if (response.data.charAt(0) == '?' || response.data.trim() == "Ok") {
					// ? seems to happen when a pin is unitialized
	...
	getPinStatus(pinId, silent = false, callback = () => { }) {
		...
		this.sendCommand([pin.commands.status, pin.pin], (error, json, rawData) => {
