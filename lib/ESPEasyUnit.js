...
module.exports = class ESPEasyUnit extends Homey.SimpleClass {
	...
	getPinList(group = false) {
		const pinList = [{
			"name": Homey.__("unit.on_board_pins"),
			"pins": require("/lib/pinlists/nodemcu-v3.json")
		}];
	...
	getPinStatus(pinId, silent = false, callback = () => { }) {
		...
		if (!pin.commands.status) {
			this.log('Pin does not have commands.status property ' + pinId);
			callback('Pin does not have commands.status property ' + pinId);
			return
		}

		this.sendCommand([pin.commands.status, pin.pin], (error, json) => {
			if (!error && !json) {
