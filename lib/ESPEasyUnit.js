...
module.exports = class ESPEasyUnit extends Homey.SimpleClass {
	...
	constructor(mac, host, port = 80, callback = () => {}) {
		...
		this.on('__log', function() {
				this.homey.app.log.call(this, `[Unit: ${this.hostname}]`, ...arguments)
			}.bind(this))
			.on('__error', function() {
				this.homey.app.error.call(this, `[Unit: ${this.hostname}]`, ...arguments)
			}.bind(this));

		this.hostname = host;
		this.port = port;
		this.isOnline = false;
		this.offlineReason = this.homey.__("offline");
		this.eventCount = 0;
		this.eventLog = {};
		this.lastEvent = null;
	...
	onInit(error, callback) {
		if (error) {
			...
			if (!this.mac) {
				this.homey.app.units.removeUnit(this);
				return;
			}
		} else {
			this.log(
			...
			var existing = this.homey.app.units.findUnit(this.mac);
			if (existing && existing != this) {
				...
				this.homey.app.units.emit('unit-initialized', this);
				callback(null, this);
			}
		}
	}
	...
	updateTelemetry(reason, recurse) {
		...
		this.homey.app.telemetry.send('Unit', reason, '/unit/' + metrics["Unit board"], metrics);
	...
	sendUnknownTaskTelemetry(reason, recurse) {
		this.getFreeTasks().forEach(task => {
			...
			if (this.homey.app.supportedTasks.includes(normalizedName)) {
				type = 'Unknown task';
				url = '/task/unknown/' + metrics["Task plugin"].replace('/', '-');
			}

			this.homey.app.telemetry.send(type, reason, url, metrics);
		});
	}
	...
	tryHost(hostname) {
		if (this.hostname == hostname) {
			...
			return this.homey.app.units.findUnit(null, hostname, 80, true);
		}
	}
	...
	setOffline(reason = this.homey.__("offline")) {
	...
	logEvent(event) {
		...
		this.eventCount = this.homey.app.safeIncrement(this.eventCount);
	...
	getPinList(group = false) {
		...
		const boardLabel = " (" + this.homey.app.getI18nString(board.name) + ")";

		const pinList = [{
			"name": this.homey.__("unit.on_board_pins") + boardLabel,
	...
	getPinStatus(pinId, silent = false, callback = () => {}) {
		...
		if (typeof pin == "undefined" || !pin.enabled) {
			...
			callback(this.this.homey.__("gpio.pin_not_found"));
			this.emit("pinUpdate", this, pinId, this.homey.__("gpio.pin_not_found"), null);
			return
		}
		...
		this.sendCommand([pin.commands.status, pin.pin], silent, (error, json, rawData) => {
			if (!error && !json && rawData.trim() == "Ok") {
				error = this.homey.__("sensor.io_board_offline");
			}
	...
	updateJSON(silent = false, callback = () => {}) {
		...
		http.get({
			...
			if (!esp || !esp.System || !esp.System['Uptime']) {
				...
				this.updateJSONCallbacks.forEach(fn => fn(this.homey.__("unit.error_connecting"), this));
				this.updateJSONCallbacks = [];
				return;
			}
