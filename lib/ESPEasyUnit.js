...
module.exports = class ESPEasyUnit extends Homey.SimpleClass {
	...
	updateTelemetry(reason, recurse) {
		...
		let metrics = {
			...
			"Unit registered": this.isRegistered() ? 1 : 0,
			"Unit tasks": this.tasks.length,
			"Unit tasks in use": this.sensors.length,
			...
			"Unit systeminfo task": this.getFreeTasks().some(task => task.TaskDeviceNumber == '26') ? 1 : 0
		};
		Homey.app.sendTelemetry('Unit', reason, '/unit/' + metrics["Unit board"], metrics);
	...
	sendUnknownTaskTelemetry(reason, recurse) {
		this.getFreeTasks().forEach(task => {
			...
			let url = '/task/unsupported/' + metrics["Task plugin"];
			if (Homey.app.supportedTasks.includes(metrics["Task plugin"])) {
				type = 'Unknown task';
				url = '/task/unknown/' + metrics["Task plugin"];
			}

			Homey.app.sendTelemetry(type, reason, url, metrics);
		});
	}
	...
	newEvent(args) {
		...
		if (this.eventCount >= Number.MAX_SAFE_INTEGER - 1)
			this.eventCount = 0;
	...
	getPinList(group = false) {
		const board = require("/lib/pinlists/boards.json")[this.detectBoardType()];
