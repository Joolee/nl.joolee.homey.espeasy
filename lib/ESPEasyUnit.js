...
module.exports = class ESPEasyUnit extends Homey.SimpleClass {
	...
	constructor(mac, host, port = 80, callback = () => {}) {
		...
		this.port = port;
		this.isOnline = false;
		this.offlineReason = Homey.__("offline");
		this.eventCount = 0;
		this.eventLog = {};
		this.lastEvent = null;
	...
	setOffline(reason = Homey.__("offline")) {
	...
	updateJSON(silent = false, callback = () => {}) {
		...
		http.get({
			...
			} else if (errorCode == "EHOSTUNREACH" || error.errno == "ECONNREFUSED") {
				this.log("Unreachable");
				this.setOffline('Unit unreachable');
			} else {
				if (error instanceof ParseError) {
					this.log('Invalid HTTP response received');
				} else {
					this.error("Unknown unit error", error.message, errorCode);
				}
				this.setOffline();
			}
