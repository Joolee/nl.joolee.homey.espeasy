...
module.exports = class GPIODriver extends GeneralDriver {
	...
	pair_getOnlineUnits(data, callback) {
		...
		for (const onlineUnit of Homey.app.units.getOnline()) {
			onlineUnits.push({
	...
	pair_getUnitPins(data, callback) {
		const unit = Homey.app.units.findUnit(data.unit.mac, data.unit.host, data.unit.port);
	...
	pair_getPinProperties(data, callback) {
		...
		const unit = Homey.app.units.findUnit(data.unit.mac, data.unit.host, data.unit.port, false);
