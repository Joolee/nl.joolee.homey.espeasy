...
module.exports = class Telemetry extends Homey.SimpleClass {
	constructor({
		...
		super();
		[this.host, this.path, this.siteId, this.pingTick, this.updateInterval] = [host, path.replace(/^\//, ''), siteId, pingTick, updateInterval];
	...
	unitInitialized(unit) {
		if (!this.appInitialize && Homey.app.units.units.length == Homey.app.units.listOnline().length) {
	...
	_setupEvents() {
		...
		// Also update the telemetry if the app failed to initialize all units in two minutes
		setTimeout(() => {
			if (!this.appInitialized) {
				this.appInitialized = true;
				Homey.app.updateTelemetry('Initialize timeout', '/app/initialize-timeout', false);
			}
		}, 120000);

		setInterval(Homey.app.updateTelemetry, this.updateInterval[0] * 1000, `${this.updateInterval[1]} update`, '/app/weeklyupdate', true);
